...
function TodoLi({ data }: TodoLiProps) {
	const [userTodo, handleCheckbox] = useHandleTodo(data);
		...
		<li className='flex w-full pb-2 border-b-2 border-slate-300 items-center mb-4 text-lg'>
			...
			{isEditMode && (
				<>
					...
					/>
					<div className='flex w-3/12 ml-2 justify-between'>
						<button className={`${BUTTON_STYLE} border-green-500`} data-testid='submit-button'>
							제출
						</button>
						<button
							// onClick={handleCancel}
							className={`${BUTTON_STYLE} border-gray-400`}
