  ...
  return (
    <form className='w-full px-10' onSubmit={handleSubmit}>
      <div className={DIV_STYLE}>
        ...
        <input className={INPUT_STYLE} type='email' data-testid='email-input' id='email' onChange={handleChange} value={account.email} placeholder='test@test.com' />
        <p className={`${TEXT_STYLE} ${account.email && !isValidate.email && 'text-red-500'}`}>이메일 형식에 맞게 입력해주세요.</p>
      </div>
      <div className={DIV_STYLE}>
        <label className={LABEL_STYLE} htmlFor='password'>
          <MdOutlineLock className='text-2xl' /> 비밀번호</label>
        <input className={INPUT_STYLE} type='password' data-testid='password-input' id='password' onChange={handleChange} value={account.password} placeholder='비밀번호를 입력해주세요' />
        <p className={`${TEXT_STYLE} ${account.password && !isValidate.password && 'text-red-500'}`}>비밀번호를 8자리 이상 입력해주세요.</p>
      </div>
